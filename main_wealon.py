...
weatherlabel = np.zeros([2131, ])
weatherfeature = np.zeros([2131, 10])
...
ar, number = np.unique(weatherlabel, return_counts=True)

weatherlabel = weatherlabel.astype(int)

weatherfeature_M = np.zeros([weatherfeature.shape[0], 5])
...
for i in range(Tmax):
    lon_train, lon_vali, lon_test, y_train, y_vali, y_test = Data_split(weatherfeature_M, weatherlabel, 0.2, 0.25, i)
    vali_best = 0
    for j in range(len(C)):
        clf = LogisticRegression(penalty='l2', C=C[j], random_state=0, multi_class='multinomial').fit(lon_train, y_train)
        clf.predict(lon_vali)
        score = np.zeros(T1)
        for k in range(T1):
            score[k] = clf.score(lon_vali, y_vali)
...
for i in range(Tmax):
    ...
    for j in range(len(C)):
        ...
        for jj in range(T1):
            clf.fit(lon_train, y_train)
            score_lin[jj] = clf.score(lon_vali, y_vali)
        clf = svm.SVC(kernel='rbf', C=C[j])
        score_rbf = np.zeros(T1)
        for jj in range(T1):
            clf.fit(lon_train, y_train)
            score_rbf[jj] = clf.score(lon_vali, y_vali)
        clf = svm.SVC(kernel='poly', C=C[j])
        score_poly = np.zeros(T1)
        for jj in range(T1):
            clf.fit(lon_train, y_train)
            score_poly[jj] = clf.score(lon_vali, y_vali)
    ...
    clf.fit(lon_train, y_train)
    s0 = clf.score(lon_test, y_test)
    clf = svm.SVC(kernel='rbf', C=c_best)
    clf.fit(lon_train, y_train)
    s1 = clf.score(lon_test, y_test)
    clf = svm.SVC(kernel='poly', C=c_best)
    clf.fit(lon_train, y_train)
    s2 = clf.score(lon_test, y_test)
...
for i in range(Tmax):
    ...
    vali_best = 0
    for j in range(len(C)):
        clf = DecisionTreeClassifier(min_samples_leaf=C[j], max_depth=10, random_state=0).fit(lon_train, y_train)
        clf.predict(lon_vali)
        score = np.zeros(T1)
        for k in range(T1):
            score[k] = clf.score(lon_vali, y_vali)
...
for i in range(Tmax):
    ...
    for j in range(len(C)):
        for k in range(len(Tr)):
            clf = RandomForestClassifier(n_estimators=Tr[k], min_samples_leaf=C[j], random_state=0).fit(lon_train, y_train)
            clf.predict(lon_vali)
            score = np.zeros(T1)
            for ii in range(T1):
                score[ii] = clf.score(lon_vali, y_vali)
            if vali_best < np.mean(score):
                vali_best = np.mean(score)
                c_bestran = C[j]
                tr_bestran = Tr[k]
...
for i in range(Tmax):
    for m in range(len(B)):
        weatherfeature_df = DF_interval(weatherfeature, B[m], feature_number)
        lon_train, lon_vali, lon_test, y_train, y_vali, y_test = Data_split(weatherfeature_df, weatherlabel, 0.2, 0.25, i)
        train_iter, vali_iter, test_iter = Data_split_torch(lon_train, lon_vali, lon_test, y_train, y_vali, y_test, batch_size)
        vali_best = 0
