...
#             vali_best = score
#             c_best[i] = C[j]
#     clf = svm.SVC(kernel='linear', C=c_best[i])
#     clf.fit(sys_train, y_train)
#     s0 = clf.score(sys_test, y_test)
#     clf = svm.SVC(kernel='rbf', C=c_best[i])
#     clf.fit(sys_train, y_train)
#     s1 = clf.score(sys_test, y_test)
#     clf = svm.SVC(kernel='poly', C=c_best[i])
#     clf.fit(sys_train, y_train)
#     s2 = clf.score(sys_test, y_test)
#     ScoreMSVM[i] = np.max((s0,s1,s2))

ScoreMSVM0 = np.zeros(Tmax)
ScoreMSVM1 = np.zeros(Tmax)
ScoreMSVM2 = np.zeros(Tmax)
for i in range(Tmax):
    sys_train, sys_vali, sys_test, y_train, y_vali, y_test = Data_split(sys_featuresM, sys_labels, 0.2, 0.25, i)
    vali_best = 0
    for j in range(len(C)):
    ...
    clf.fit(sys_train, y_train)
    ScoreMSVM0[i] = clf.score(sys_test, y_test)
...
for i in range(Tmax):
    ...
    clf.fit(sys_train, y_train)
    ScoreMSVM1[i] = clf.score(sys_test, y_test)
...
for i in range(Tmax):
    ...
    clf.fit(sys_train, y_train)
    ScoreMSVM2[i] = clf.score(sys_test, y_test)

ScoreMSVM = ScoreMSVM0
if np.mean(ScoreMSVM) < np.mean(ScoreMSVM1):
    ScoreMSVM = ScoreMSVM1
elif np.mean(ScoreMSVM) < np.mean(ScoreMSVM2):
    ScoreMSVM = ScoreMSVM2
...
Scoredfsvm0 = np.zeros(Tmax)
Scoredfsvm1 = np.zeros(Tmax)
Scoredfsvm2 = np.zeros(Tmax)
for i in range(Tmax):
    sys_train, sys_vali, sys_test, y_train, y_vali, y_test = Data_split(sys_featuresdf, sys_labels, 0.2, 0.25, i)
    vali_best = 0
    for j in range(len(C)):
    ...
    clf = svm.SVC(kernel='linear', C=c_best)
    clf.fit(sys_train, y_train)
    Scoredfsvm0[i] = clf.score(sys_test, y_test)
...
for i in range(Tmax):
    ...
    clf = svm.SVC(kernel='rbf', C=c_best)
    clf.fit(sys_train, y_train)
    Scoredfsvm1[i] = clf.score(sys_test, y_test)
...
for i in range(Tmax):
    ...
    clf = svm.SVC(kernel='poly', C=c_best)
    clf.fit(sys_train, y_train)
    Scoredfsvm2[i] = clf.score(sys_test, y_test)

Scoredfsvm = Scoredfsvm0
if np.mean(Scoredfsvm) < np.mean(Scoredfsvm1):
    Scoredfsvm = Scoredfsvm1
elif np.mean(Scoredfsvm) < np.mean(Scoredfsvm2):
    Scoredfsvm = Scoredfsvm2
